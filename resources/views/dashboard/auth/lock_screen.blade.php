...
@section('content')
    <div class="card card-round card-shadowed px-50 py-30 w-400px mb-0 text-center" style="max-width: 100%">
        ...
        <form class="form-type-line " action="{{ route('admin.users.lock') }}" method="post">
            @csrf
            ...
            <button class="btn btn-bold btn-block btn-primary" type="submit">
                <i class="fa fa-unlock-alt mr-2"></i> Unlock
            </button>
        </form>

        <p class="text-center text-muted fs-13 mt-20">
            You're not {{ Auth::user()->name }} ?
            <a class="text-primary fw-500" href="{{ route('logout') }}"
               onclick="event.preventDefault();document.getElementById('logout-form').submit();">Sign in</a>
        </p>
        <form id="logout-form" action="{{ route('logout', ['redirect' => 'login']) }}" method="POST"
              style="display: none;">
            @csrf
        </form>
    </div>
